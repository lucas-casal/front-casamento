...
export default function CreatePostForm(props: FormProps) {
    ...
    const names = localStorage.getItem('GL') ? JSON.parse(localStorage.getItem('GL')!) : getNames()
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (error.response?.status === 404) {
                ...
                alert(`Não foi possível encontrar ${(notFoundArray.length === 1) ? message :
    ...
    return (
        <>
            <form className='absolute flex py-10 flex-col items-center justify-center h-auto w-full'>
                <h1 className="w-full self-center text-center font-gwendolyn text-5xl text-black">Confirmação de presença</h1>
